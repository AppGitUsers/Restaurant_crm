...
function SettingRow({ icon: Icon, label, hint, children }) {
  return (
    <tr className="border-b border-gray-100 last:border-0">
      ...
      <td className="py-3.5">{children}</td>
    </tr>
  )
}
        ...
        <table className="w-full">
          <tbody>
            <SettingRow icon={Building2} label="Company Name" hint="Used in PDF bills and WhatsApp messages">
              <input className="input w-full max-w-sm" value={form.company_name || ''} onChange={e => f('company_name', e.target.value)} placeholder="My Restaurant" />
            </SettingRow>
            <SettingRow icon={Phone} label="Company Phone">
              <input className="input w-full max-w-sm" value={form.company_phone || ''} onChange={e => f('company_phone', e.target.value)} placeholder="+91 98765 43210" />
            </SettingRow>
            <SettingRow icon={Mail} label="Company Email">
              <input type="email" className="input w-full max-w-sm" value={form.company_email || ''} onChange={e => f('company_email', e.target.value)} placeholder="contact@restaurant.com" />
            </SettingRow>
            <SettingRow icon={MapPin} label="Address" hint="Full address shown on invoices">
              <textarea className="input w-full max-w-sm resize-none" rows={2} value={form.company_address || ''} onChange={e => f('company_address', e.target.value)} placeholder="123 Food Street, City - 600001" />
            </SettingRow>
            <SettingRow icon={Hash} label="GSTIN" hint="GST Identification Number">
              <input className="input w-full max-w-sm font-mono uppercase" value={form.gstin || ''} onChange={e => f('gstin', e.target.value.toUpperCase())} placeholder="22AAAAA0000A1Z5" maxLength={15} />
            </SettingRow>
          </tbody>
        </table>
      </div>
      ...
      <div className="card">
        ...
        <table className="w-full border border-gray-200 rounded-xl overflow-hidden text-sm">
          <thead className="bg-gray-50">
            <tr>
              <th className="text-left px-4 py-3 text-gray-500 font-semibold w-56">Field</th>
          ...
          <tbody>
            <tr className="border-t border-gray-100">
              ...
              <td className="px-4 py-3 text-gray-400 text-xs">Applied to every new bill as the default tax percentage</td>
            </tr>
            <tr className="border-t border-gray-100">
              ...
              <td className="px-4 py-3 text-gray-400 text-xs">Printed on tax invoices for GST compliance</td>
            </tr>
          </tbody>
        </table>
        ...
        <table className="w-full">
          <tbody>
            ...
            <SettingRow icon={Hash} label="Phone Number ID" hint="From Meta → WhatsApp → Getting Started">
              <input
                className="input w-full max-w-sm font-mono"
        ...
        {[
          ...
          <div key={label} className="card flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-xs text-gray-400">{label} notifications</div>
            </div>
          </div>
        ))}
      </div>
            ...
            {rows.map(n => (
              <tr key={n.id}>
                ...
                <td className="max-w-xs">
                  <p className="text-sm text-gray-700 line-clamp-2 whitespace-pre-line">{n.message}</p>
                  {n.error_message && (
